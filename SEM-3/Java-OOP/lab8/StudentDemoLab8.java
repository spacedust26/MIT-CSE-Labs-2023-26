...
class SeatsFilledException extends Exception{
   SeatsFilledException(int count){
    super("Seats full"+ count + ">25");
   }
}
class studentrec{
    int reg;
    ...
    studentrec(String name, String doj, short sem, float gpa, float cgpa) throws SeatsFilledException{
        count+=1;
        if(count>25) throw new SeatsFilledException(count);
        this.name =name;
        this.doj= doj;
        this.sem = sem;
        this.gpa=gpa;
...
class StudentDemoLab8 {
    public static void main(String [] args){
        ...
        catch(SeatsFilledException e){System.out.println(e); return;}
        for(int i=0;i<n;i++){
            sr[i].display();
        }
    }
